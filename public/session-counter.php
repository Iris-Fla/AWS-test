...
// 値の保存
if (isset($session_values["counter"])) {
} else {
    $session_values["counter"] = 0;
}

if (isset($session_values["access_date"])) {
    $last_access = $session_values["access_date"];

} else {
    $last_access = "初アクセスやで。";
}

$session_values["access_date"] = date("Y/m/d H:i:s");
...
echo "カウントは";
echo $session_values["counter"];
echo 'やで<br>さいごのアクセスは・・・・';
echo $last_access;
?>
